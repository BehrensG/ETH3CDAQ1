...
/*
 * SYSTem:SERVice:EEPROM {RESET|DEFault}
 *
 * @INFO:
...
scpi_result_t SCPI_SystemServiceID(scpi_t * context)
{
	...
	strncpy(bsp.eeprom.structure.info.manufacturer,buffer,SCPI_MANUFACTURER_STRING_LENGTH);
	strncpy(bsp.eeprom.structure.info.device,buffer,SCPI_DEVICE_STRING_LENGTH);
